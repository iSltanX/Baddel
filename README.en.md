...
<picture>
  ...
  <img alt="Baddel — بدّل: Typed it in the wrong language? Switch it." src="docs/assets/cover-light.png" width="100%">
</picture>

**Typed «اثممخ» and meant `hello`? One shortcut fixes it in place, and switches the keyboard layout so you can keep typing.**

[Install](#install) · [How it works](#how-it-works) · [Privacy](#privacy) · [FAQ](#frequently-asked-questions) · [العربية](README.md)
...
[![UI](https://img.shields.io/badge/UI-Arabic%20%C2%B7%20English-3F5673?style=flat-square)](#why-baddel)
...
| Permission | Why |
| --- | --- |
| **Accessibility** | To read the selected text or the word before the cursor when you press the shortcut, replace it, and send copy/paste keystrokes in apps that don't expose their text. |

This is the only permission. **It does not request** Input Monitoring, Screen Recording, or Full Disk Access. The global shortcut is registered through the system's shortcuts interface, so Baddel doesn't need to monitor the keyboard.
...
Baddel has no reason to know what you type, and no way to send it anywhere:
...
All the app's logic and state live in Rust; the UI only displays. The menu and notice are native, not webview, so no webview process stays alive at idle. Changelog in [CHANGELOG.md](CHANGELOG.md).
